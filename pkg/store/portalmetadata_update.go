...
func (pmu *PortalMetadataUpdate) check() error {
	...
	if _, ok := pmu.mutation.PortalID(); pmu.mutation.PortalCleared() && !ok {
		return errors.New(`store: clearing a required unique edge "PortalMetadata.portal"`)
	}
	return nil
}
...
// SetPortalID sets the "portal" edge to the Portal entity by ID.
func (pmuo *PortalMetadataUpdateOne) SetPortalID(id uint32) *PortalMetadataUpdateOne {
...
func (pmuo *PortalMetadataUpdateOne) sqlSave(ctx context.Context) (_node *PortalMetadata, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`store: missing "PortalMetadata.id" for update`)}
	}
