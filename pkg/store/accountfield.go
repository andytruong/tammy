	...
	for i := range columns {
		...
		case accountfield.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				af.account_fields = new(uint32)
				*af.account_fields = uint32(value.Int64)
			}
		}
	}
	return nil
...
func (af *AccountField) Unwrap() *AccountField {
	...
	if !ok {
		panic("store: AccountField is not a transactional entity")
	}
