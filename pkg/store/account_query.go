...
func (aq *AccountQuery) QueryFields() *AccountFieldQuery {
	query := &AccountFieldQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AccountQuery) WithFields(opts ...func(*AccountFieldQuery)) *AccountQuery {
	query := &AccountFieldQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	...
	if query := aq.withUser; query != nil {
		...
		nodeids := make(map[uint32][]*Account)
		for i := range nodes {
	...
	if query := aq.withFields; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[uint32]*Account)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Fields = []*AccountField{}
		}
		query.withFKs = true
		query.Where(predicate.AccountField(func(s *sql.Selector) {
			s.Where(sql.InValues(account.FieldsColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.account_fields
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "account_fields" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "account_fields" returned %v for node %v`, *fk, n.ID)
			}
