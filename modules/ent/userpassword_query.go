...
func (upq *UserPasswordQuery) IDs(ctx context.Context) ([]int, error) {
	var ids []int
...
//		GroupBy(userpassword.FieldHashedValue).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.UserPassword.Query().
//		Select(userpassword.FieldHashedValue).
//		Scan(ctx, &v)
//
...
func (upq *UserPasswordQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserPassword, error) {
	...
	if query := upq.withUser; query != nil {
		...
		nodeids := make(map[int][]*UserPassword)
		for i := range nodes {
