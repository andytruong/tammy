...
type UserQuery struct {
	...
	// eager-loading edges.
	withPassword *UserPasswordQuery
...
func (uq *UserQuery) QueryPassword() *UserPasswordQuery {
	query := &UserPasswordQuery{config: uq.config}
...
func (uq *UserQuery) QueryEmails() *UserEmailQuery {
	query := &UserEmailQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithPassword(opts ...func(*UserPasswordQuery)) *UserQuery {
	query := &UserPasswordQuery{config: uq.config}
...
func (uq *UserQuery) WithEmails(opts ...func(*UserEmailQuery)) *UserQuery {
	query := &UserEmailQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withPassword; query != nil {
		...
		query.Where(predicate.UserPassword(func(s *sql.Selector) {
			s.Where(sql.InValues(user.PasswordColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_password
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_password" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_password" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Password = n
		}
	}
	...
	if query := uq.withEmails; query != nil {
		...
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Emails = []*UserEmail{}
		}
		query.withFKs = true
