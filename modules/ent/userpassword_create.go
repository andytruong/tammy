...
func (upc *UserPasswordCreate) SetUserID(id int) *UserPasswordCreate {
...
func (upc *UserPasswordCreate) check() error {
	if _, ok := upc.mutation.HashedValue(); !ok {
		return &ValidationError{Name: "hashedValue", err: errors.New(`ent: missing required field "UserPassword.hashedValue"`)}
	}
	if _, ok := upc.mutation.IsActive(); !ok {
		return &ValidationError{Name: "isActive", err: errors.New(`ent: missing required field "UserPassword.isActive"`)}
	}
	if _, ok := upc.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "createdAt", err: errors.New(`ent: missing required field "UserPassword.createdAt"`)}
	}
	if _, ok := upc.mutation.UserID(); !ok {
		return &ValidationError{Name: "user", err: errors.New(`ent: missing required edge "UserPassword.user"`)}
	}
	return nil
}
