...
func toProtoAccount(e *ent.Account) (*Account, error) {
	...
	v.IsBlocked = isBlocked
	for _, edg := range e.Edges.Fields {
		id := int64(edg.ID)
		v.Fields = append(v.Fields, &AccountField{
			Id: id,
		})
	}
	for _, edg := range e.Edges.Portal {
		id := int64(edg.ID)
		v.Portal = append(v.Portal, &Portal{
			Id: id,
		})
	}
...
func (svc *AccountService) Get(ctx context.Context, req *GetAccountRequest) (*Account, error) {
	...
	case GetAccountRequest_WITH_EDGE_IDS:
		get, err = svc.client.Account.Query().
			Where(account.ID(id)).
			WithFields(func(query *ent.AccountFieldQuery) {
				query.Select(accountfield.FieldID)
			}).
...
func (svc *AccountService) Update(ctx context.Context, req *UpdateAccountRequest) (*Account, error) {
	...
	accountIsActive := account.GetIsActive()
	m.SetIsActive(accountIsActive)
	accountIsBlocked := account.GetIsBlocked()
	m.SetIsBlocked(accountIsBlocked)
	for _, item := range account.GetFields() {
		fields := int(item.GetId())
		m.AddFieldIDs(fields)
	}
	for _, item := range account.GetPortal() {
		portal := int(item.GetId())
		m.AddPortalIDs(portal)
	}
